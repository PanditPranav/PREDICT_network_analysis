...
def generete_temp_network(virus, hosts, ViralFamily, PubMed, BPnx, Gc_complete, virus_df):
    temp_BPnx = BPnx.copy() ## making a copy of original Bipartite network
    #print (temp_BPnx.number_of_nodes()) ## checking number of nodes
    #virus_nodes = []#creating list of virus nodes from bipartite network
    #for n, v in temp_BPnx.nodes(data=True):
    #    if v['type'] == 'virus':
    #        virus_nodes.append(n)
    ...
    for index, row in new_edges.iterrows():
            if row['n_shared_hosts'] > 0:
                Gc_complete_temp.add_edge(row['Virus1'], row['Virus2'], weight = row['n_shared_hosts'], hosts = ','.join(row['shared_hosts']))
    ...
    return Gc_complete_temp, edges_to_predict, virus_df_temp, n_e
...
def generete_temp_network_known(virus, hosts, ViralFamily, PubMed, BPnx, Gc_complete, virus_df):
    temp_BPnx = BPnx.copy() ## making a copy of original Bipartite network
...
def preprocessing_x(data_frame, network, virus_df_temp, virus_df):
    #print('this function is in prediction file')
    #predictors = [
...
def prediction(temp_x, clf_binary, plot = False):
    prediction = pd.DataFrame(clf_binary.predict_proba(temp_x))
    prediction['Binary'] = clf_binary.predict(temp_x)
    prediction.columns = ['Probability_0', 'Probability_1', 'Prediction']
    if plot:
        fig, (ax1) = plt.subplots(figsize = [6,6])
        prediction.Probability_1.hist(ax = ax1)
        ax1.set_xlabel('Probability')
        ax1.set_ylabel('Frequency')
        ax1.axvline(x=0.7, color='r')
        ax1.set_title('Probability of new virus to be found in the same host of a known virus\n (Shared links with known viruses)') 
        fig.tight_layout()
        plt.show()
    return prediction
...
def run_predictions(virus, hosts, PubMed, ViralFamily, BPnx, Gc_complete, virus_df,
    ...
    #Gc_complete_temp, edges_to_predict, virus_df_temp, new_edges
    temp_Gc, to_predict, virus_df_temp, new_edges_temp = generete_temp_network(virus = virus, 
                                                               hosts =hosts,
                                                               PubMed = PubMed,
...
def run_predictions_known(virus, hosts, PubMed, ViralFamily, BPnx, Gc_complete, virus_df,
    ...
    ## to predict is is the dataframe for for observed edges, will be used for comparison /validation during corss validation in cases for known viruses
    #Gc_complete_temp, edges_to_predict, virus_df_temp, new_edges
    temp_Gc, to_predict, virus_df_temp, new_edges_temp = generete_temp_network_known(virus = virus, 
                                                               hosts =hosts,
                                                               PubMed = PubMed,
                                                               ViralFamily = ViralFamily, 
                                                               BPnx = BPnx, Gc_complete = Gc_complete, virus_df = virus_df)
...
def run_predictions_validation(virus, hosts, PubMed, ViralFamily, BPnx, Gc_complete, virus_df,
                    clf_multi, inv_dictionary):
    
    ## creates a new network by adding the new virus and generating edges for the newly found virus based on the host. Used BPnx for it
    ## to predict is is the dataframe for for observed edges, will be used for comparison /validation during corss validation in cases for known viruses
    
    temp_Gc, to_predict, virus_df_temp = generete_temp_network(virus = virus, 
                                                               hosts =hosts,
                                                               PubMed = PubMed,
                                                               ViralFamily = ViralFamily, 
                                                               BPnx = BPnx, Gc_complete = Gc_complete, virus_df = virus_df)
    ...
    result = prediction_validation_multiclass(temp_x= temp_x,
                        clf_multi= clf_multi,
